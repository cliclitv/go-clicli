...
func AddComment(pos int, content string, pid int, uid int, cid int) (*Comment, error) {
	t := time.Now()
	ctime := t.Format("2006-01-02 15:04")
	stmtIns, err := dbConn.Prepare("INSERT INTO comments (pos,content,time,pid,uid,cid) VALUES ($1,$2,$3,$4,$5,$6)")
	if err != nil {
		return nil, err
	}
	_, err = stmtIns.Exec(pos, content, ctime, pid, uid, cid)
	if err != nil {
		return nil, err
	}
	defer stmtIns.Close()

	res := &Comment{Pos: pos, Content: content, Time: ctime, Uid: uid, Pid: pid, Cid: cid}
	return res, err
}
...
func GetComments(pid int, uid int, cid int, page int, pageSize int) ([]*Comment, error) {
	...
	query = `SELECT comments.id,comments.pos,comments.content,comments.time,comments.pid,comments.cid,users.id,users.name,users.qq FROM comments INNER JOIN users ON comments.uid = users.id 
	...
	for rows.Next() {
		var id, pid, uid, pos, cid int
		var content, ctime, uname, uqq string
		if err := rows.Scan(&id, &pos, &content, &ctime, &pid, &cid, &uid, &uname, &uqq); err != nil {
			return res, err
		}

		c := &Comment{Id: id, Pos: pos, Content: content, Time: ctime, Pid: pid, Cid: cid, Uid: uid, Uname: uname, Uqq: uqq}
		res = append(res, c)
	}
	return res, nil
