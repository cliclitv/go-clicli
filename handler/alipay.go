...
func Pay(name string, order string, price string) interface{} {
	ctime := time.Now().In(time.FixedZone("CST", 8*3600)).Format("2006-01-02 15:04:05")
	p := `app_id=2021003130695981&biz_content={"subject":"` + name + `","body":"老黑-视频标题-uid","out_trade_no":` + order + `,"total_amount":` + price + `}&charset=UTF-8&method=alipay.trade.precreate&notify_url=http://www.qq.com&sign_type=RSA2&timestamp=` + ctime + "&version=1.0"
	p2 := `app_id=2021003130695981&biz_content=` + url.QueryEscape(`{"subject":"`+name+`","body":"老黑-视频标题-uid","out_trade_no":`+order+`,"total_amount":`+price+`}`) + `&charset=UTF-8&method=alipay.trade.precreate&notify_url=` + url.QueryEscape(`http://www.qq.com`) + `&sign_type=RSA2&timestamp=` + url.QueryEscape(ctime) + "&version=1.0"
	fmt.Printf("p: %v\n", p)
