...
func Pay(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	p := `app_id=2021003130695981&biz_content={"subject":"CliCli超大会员","out_trade_no":"888","total_amount":6.6}&charset=UTF-8&method=alipay.trade.precreate&notify_url=http://www.qq.com&sign_type=RSA2&timestamp=` + ctime + "&version=1.0"
	p2 := `app_id=2021003130695981&biz_content=` + url.QueryEscape(`{"subject":"CliCli超大会员","out_trade_no":"888","total_amount":6.6}`) + `&charset=UTF-8&method=alipay.trade.precreate&notify_url=` + url.QueryEscape(`http://www.qq.com`) + `&sign_type=RSA2&timestamp=` + url.QueryEscape(ctime) + "&version=1.0"
	fmt.Printf("p: %v\n", p)
